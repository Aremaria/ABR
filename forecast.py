...
year_outliers = (2020, 2021) # выбросы

def DDDformula(df_temp, bd, ab): # формула для прогноза DDD
    # для прогноза нужен коэфф - считаем на нормированном ряде (полных данных DDD)
    y = np.array(df_temp['Actual']/df_temp['Actual'].mean()).reshape(len(df_temp), 1)
    ...
    df_forecast = pd.concat([pd.DataFrame({'AntibioticClass': [ab], 'TypeBD': [bd]}), df_forecast], axis=1)

    return row, df_forecast

def DDDForecast(): # прогнозирование всех потреблений
    # коэффициэнты строились по полным данным, модельные данные чуть урезаны (match по регионам) - ряды д.б. нормированы
    ...
    for bd in bd_list:
        for ab in list_Ab:
            df_temp = (DDDstat[['Year', 'AntibioticClass', bd]]
                       .loc[DDDstat['AntibioticClass'] == ab]
                       .reset_index(drop=True)
                       .drop(columns='AntibioticClass')
                       .rename(columns={bd: 'Actual'})
                       .fillna(0))
            ...
            if len(df_temp) >= 5: # если ряд достаточно длинный
                ...
                val_adj = df_temp.loc[df_temp['Year'].isin([2019, 2021]), 'Actual'].mean() # среднее вместо выбросов
                df_adj = df_temp.copy()
                df_adj.loc[df_adj['Year'].isin(year_outliers), 'Actual'] = val_adj

                row_adj, df_forecast_adj = DDDformula(df_adj, bd, ab) # прогноз по очищенному ряду
...
def PointForecast(last, df_ABR_last, dict_forec, model, scaler, pca_model, typeddd): # прогноз для 1 точки
    datapoint = last.copy() 
    year = max(last['Year'])
    ...
    for bd in bd_list:
        for ab in dict_forec['DictDDD'][bd]['InitDDD']:
            ...
            datapoint[col_prev] = last[col]
        
    # обновление текущего ddd - прогноз ddd
    if typeddd in ('no adj', 'adj'): # если какой-то вариант прогноза по ddd (линейный с выбросами или без)
        ...
        # обновление ddd на среднее по регионам для накрутки коэффициента
        datapoint = datapoint.drop(columns=set(ddd_mean.columns.to_list()))
        ...
        for bd in bd_list:
            ...
            for ab in dict_forec['DictDDD'][bd]['InitDDD']:   
                ...
                ab_base_filtered = ab_coeff_base.loc[ab_coeff_base['AntibioticClass'] == ab]
                a = ab_base_filtered[a_name].values[0]
                const = ab_base_filtered[const_name].values[0]

                # ddd не менее нуля, прогнозирруем ddd для новой точки, * на среднеее по региону
                coeff = max(a * (year + 1) + const, 0)
                datapoint[col] = datapoint[col] * coeff
    ...
    piv_ddd = pd.pivot_table(datapoint, values=ddd_columns_cur, index=['Year', 'RegionName'],
                             aggfunc='mean').reset_index() # усреднение по регионам, так как переход от классификатора
...
def HorizontForecast(horizont, df_ABR_pair, dict_forec, model, scaler, pca_model, typeddd, list_ab):
    ...
    for point in range(1, horizont+1): 
        ...
        df_ABR_last = pd.concat([df_ABR_last, new_abr]) # добавление новой точки для обновления расчета MovAvg
...
def PairForecast(df_ABR_model, data, dict_model, ab, org, ab_coeff, horizont, typeddd, typeparam): # для пары
    ...
    # для прогноза ddd в разрезе регионов через единый коэффициент
    ddd_region_mean = pd.pivot_table(data, values=col_val, index=['RegionName'], aggfunc='mean')
    ddd_region_mean_adj = pd.pivot_table(data.loc[data['Year'].isin(year_outliers) == False],
                                         values=col_val, index=['RegionName'], aggfunc='mean')

    base = MakeBaseForecast(data, dict_model, dict_model['model_columns']) # база для прогноза (последний год)
    
    dict_forec = copy.deepcopy(dict_model)  
    dict_forec.update({'ddd_region_mean': ddd_region_mean,
                       'ddd_region_mean_adj': ddd_region_mean_adj, 
                       'ab_coeff': ab_coeff, 'base' : base})
    
    list_ab = np.unique(dict_forec['DictDDD']['HOSPITAL']['InitDDD'] +  dict_forec['DictDDD']['RETAIL']['InitDDD'])
    ind_col = ['OrganismName', 'AntibioticName', 'AntibioticClass', 'RegionName', 'Year', 'Nosocomial']

    # прогноз в минимальной детализаци
    result, result_ddd = HorizontForecast(horizont, df_ABR_pair, dict_forec, model, scaler, pca_model, typeddd, list_ab)
